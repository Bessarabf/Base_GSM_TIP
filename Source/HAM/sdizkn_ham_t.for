! correction O2 before turbopause
     ...
     *                  solu,gkoor,delta,nsu,dtet,uts,ddolg)
      USE mo_gsm_const, ONLY:amo2,amn2,amo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! correction O2 to turbo pause !!!!!!!!!!!!!!!!!!!!!!!!!!!!
       do  i=2,n11
        do  j=1,n2
         do  k=2,6 
           aNall=ro1(i,j,k)/(0.21*amO2+0.79*amN2)
           an11(i,j,k)= 0.21*aNall
         end do
        end do
       end do
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! end correction 
    ...
    1  continue
       if (key.eq.1) then
          print *,' N2 subtract until',lkk, ' point',ii,jj
           ...
           print *,'lkk=',lkk,an21(1,1,lkk),an11(1,1,lkk),an31(1,1,lkk)
       end if
