...
c
      subroutine nonew_eddy(pgl,pgi,gkoor,eddyco,rads,rp,g,
...
!!!!
     ...
     *          rads(nh),rp(nh),g(nh),eddyco(nh,its),gkoor(2,its,ids)
...
! 
!  progonka N(2D)
          call ndprog_eddy(cNd,cNo,cNoi,cN2i,cNe
     *            ,pgl,eddyco,rads,rp,g,kpars,nh,its,ids,i,j,hi,dt)
!  progonka N(4S)
       
    	  call nprog_eddy(cNd,cNoi,cO2i,cN2i,cNe
     *            ,pgl,eddyco,rads,rp,g,kpars,nh,its,ids,i,j,hi,dt)
! progonka NO. Altitude part
	   call noprog_eddy(cNd,cNo,cO2i
     *            ,pgl,eddyco,rads,rp,g,kpars,nh,its,ids,i,j,hi,dt)
          end do
	end do	  
...
c . . . for GSM [N4S)] диффузионная постановка
      subroutine nnewp_eddy(cNd,cNo,cNoi,cO2i,cNe
     *                ,pgl,eddyco,rads,rp,g,kpars,nh,its,ids,i,j,hi,dt)
      dimension pgl(kpars,nh,its,ids),cNd(its,ids,nh),
     *          cNoi(nh), cO2i(nh),cNe(nh),cNo(nh)
     *         ,eddyco(nh,its),rads(nh),rp(nh),g(nh)
     *         
...
c    . . . Coef. Mol. Dif.
        ...
        r(k)=cmd(k)*(1./h+crab)+eddyco(k,i)*(1./hsr+crab)
...
cc      r(k)=r(k)-pgl(10,k,i,j)
   1  continue
     ...
     *     amn2*pgl(2,1,i,j))/sum
      ...
      r(1)=cmd(1)*(1./h+crab)+eddyco(1,i)*(1./hsr+crab)
      do k=1,nh-1
        cmd05=(cmd(k+1)+cmd(k)+eddyco(k+1,i)+eddyco(k,i))*.5
        r05=(r(k)+r(k+1))*.5
c       if(k.eq.n-1) then
...
c . . . for GSM
      subroutine ndnew_eddy(cNd,cNo,cNoi,cN2i,cNe
     *                ,pgl,kpars,nh,its,ids,i,j,key,dt)
...
!	  write(*,*) k,dim(k)
	...
	! NO profile
      subroutine noprof_eddy(cNd,cNo,cO2i
     *             ,pgl,eddyco,rads,rp,g,kpars,nh,its,ids,i,j,hi,dt)
