cc                       CYCLT1
c   23/03/2019 - ver O2+ & NO+ write in f4
c   _HAM - integrate with HAMMONIA 
...
c    version 25.03.2012 - bmod and isat
c    
      subroutine cyclt1_ham(god,day,ut0,ut1,dtt,dts,tau,solen,sole,solu,
...
!!!   electron precipitation
      ! Zhang & Paxton
      if(mass(12).eq.2.or.mass(12).eq.4.or.mass(12).eq.6)then
...
234            format(' cyclt1 : ********  ERROR ! ********** '/
     ...
     *                   pole,nv,idt,vert,mast)
              call cyclt2_ham(mast,ntsl,nl,par,nr,pari,ni,par1,PAR2,b,c,
    ...
    4   continue
        ...
        if(mass(13).ne.0) then
         call rvert(nf,kpart,ddolgt,ntsl,nl,kdf,ldor,isp,
     *               pole,nv,idt,vert,mast)
         call cyclt2_ham(mast,ntsl,nl,par,nr,pari,ni,par1,PAR2,b,c,its,
...
c . . . writing date and time into inf-record
      read(4,rec=1) pole
      pole(2)=god
      pole(3)=day
      pole(4)=ut0
