!!!!  NEWTON ITERATION
      subroutine molio3Nit(cO2pl,cNOpl,par,ids,its,nh,kpars,pari,ins,
     *                  mass,dts,j,ntr,key) 
	                 
      dimension cO2pl(nh,its,ids),cNOpl(nh,its,ids),par(kpars,nh,its),
...
!
	...
	! Файл для сохранения тестовых значений параметров

!	OPEN(10,FILE='Molion.txt')
        
	do ig=1,its-1 
		do k=1,nh
		! подготовка одномерных массивов 
	          cO2=par(1,k,ig)    ! cO2	     
	          cN2=par(2,k,ig)    ! cNO 
                  cO=par(3,k,ig)     ! cO 
		  cNO=par(4,k,ig)    ! cNO
	! !!!!!!!!!!
	          ...
	          te=par(9,k,ig)
	          qO2=par(13,k,ig) ! скорости ионзации
	          qN2=par(14,k,ig)
	          qNO=par(15,k,ig)
	          qO=par(16,k,ig)
 ...
 !             key=0
     ...
     *		      al(7)*cn3I*cN2+al(8)*cn4I*cO
              F1=cn1i*(1+pL1*dts)-cO2pl(k,ig,j)-Q1*dts
              F2=cn2i*(1+pL2*dts)-cNOpl(k,ig,j)-Q2*dts
...
!!!!   correction negative values
           if(cn1I.lt.0.) cn1I=cO2pl(k-1,ig,j)
           if(cn2I.lt.0.) cn2I=cNOpl(k-1,ig,j)   
           cO2pl(k,ig,j)=cn1I
           cNOpl(k,ig,j)=cn2I
 !          write(10,*) ig,k,j,cO2pl(k,ig,j),cNOpl(k,ig,j),dnorm,it
           par(6,k,ig)=cO2pl(k,ig,j)+cNOpl(k,ig,j)+cn4I
