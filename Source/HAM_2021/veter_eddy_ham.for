...
c vn_fi
c vrprim_s
      subroutine veter_ham(vi1,vj1,vi,vj,vr,vim,vjm,an1,an2,an3,an6,
     *                 ro,pgl,pgi,rp,r,eddyco,n,n1,n2,kpars,ins,dt)
!
      USE mo_ham_gsm, ONLY: dragViGSM, dragVjGSM
...
c     . . . Расщепление по вертикали
     ...
     *          pgl,pgi,rp,r,eddyco,n,n1,n2,kpars,ins,dt,dragViGSM)
...
c     . . . Расщепление по вертикали
     ...
     *          pgl,pgi,rp,r,eddyco,n,n1,n2,kpars,ins,dt,dragVjGSM)
      return
      end
...
! ion drag to HAMMONIA in m/c2
            dragViGSM(i,j,k)=caion*(pgi(3,k,i,j)-pgl(11,k,i,j))! vi(i,j,k))+
     *                      +cmion*(vim(k,i,j)-pgl(11,k,i,j))!vi(i,j,k))
            dragViGSM(i,j,k)=dragViGSM(i,j,k)/ro(i,j,k)*.01
! ion drag to HAMMONIA end
...
! ion drag to HAMMONIA
            dragVjGSM(i,j,k)=caion*(pgi(4,k,i,j)-pgl(12,k,i,j)) !vj(i,j,k))+
     *                      +cmion*(vjm(k,i,j)-pgl(12,k,i,j))  !vj(i,j,k))
            dragVjGSM(i,j,k)=dragVjGSM(i,j,k)/ro(i,j,k)*.01
! ion drag to HAMMONIA end
